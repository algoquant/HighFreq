...
#' function as '\code{\link[qmao]{do.call.rbind}}' from package 'qmao'.
#' 
#' @param list_var A list of \code{vectors}, \code{matrices}, \code{data
#'   frames}, or \code{time series}.
#' @return  a single \code{vector}, \code{matrix}, \code{data frame}, or
#'   \code{time series}.
#' @details Performing a \code{do.call(rbind, x_ts)} on a list of 'xts' time
#'   series is very slow, and often causes an "out of memory" error.
#' @examples
...
#' @details Extreme values are those that exceed a fixed multiple of the running
#'   volatility.
#' @details Extreme values are the very tips of the tails when the distribution
#'   of values becomes very fat-tailed.
#' @examples
#' # create xts time series
#' x_ts <- xts(x=rnorm(1000), order.by=(Sys.time()-3600*(1:1000)))
...
#' 
#' Calculates the running volatility of simple returns, defined as a quantile
#' over a sliding window. Identifies jump values as those returns that exceed
#' the scaled running volatility, but their sum with the lagged returns doesn't
#' exceed it.
#' 
#' @param time_series univariate \code{xts} time series.
#' @inheritParams extreme_values
#' @return  \code{logical vector}.
#' @details Jump values are those where neighboring values both exceed the
#'   scaled running volatility (defined as a quantile), but their sum doesn't
#'   exceed it.
#' @details Jump values are simply isolated prices out of proportion to their
#'   neighbors.
#' @examples
#' # create xts time series
#' x_ts <- xts(x=rnorm(1000), order.by=(Sys.time()-3600*(1:1000)))
...
#' 
#' Calculate mid prices from bid and offer TAQ data, scrub them and cbind with
#' volume data, and aggregate to 'period' OHLC data.
#' 
#' @param taq_data \code{TAQ xts} time series.
#' @inheritParams extreme_values
#' @param period aggregation period.
#' @return  \code{OHLC xts} time series.
#' @details Valid 'period' character strings include: "minutes", "3 min", "5
#'   min", "10 min", "15 min", "30 min", and "hours".
#' @details The time index is rounded up to the next integer multiple of
#'   'period'.
#' @examples
...
# select essential columns
  taq_data <- taq_data[, c("Bid.Price", "Ask.Price", "Trade.Price", "Volume")]
...
#  colnames(mid_prices) <- "Mid.Price"
# replace suspect values with NA, and perform 'locf'
  taq_data[jump_values(mid_prices), ] <- NA
  taq_data <- na.locf(taq_data)
#  taq_data <- na.locf(taq_data, fromLast=TRUE)
  taq_data
}  # end scrub_TAQ
...
#' @param data_dir \code{character string} directory containing input *.RData files.
#' @param output_dir \code{character string} directory containing output *.RData files.
#' @return  \code{OHLC xts} time series.
#' @details The high frequency data is assumed to be stored in separate
#'   directories for each symbol, containing *.RData files, each file with one
#'   day of data.
#' @details Each symbol has its own directory, and the symbol directories reside
#'   in 'data_dir'.
#' @details The xts data is loaded, scrubbed, rbinded, and aggregated to OHLC
#'   format.
#' @details The OHLC data is assigned to a variable named 'sym_bol', and then
#'   saved to a *.RData file in the 'cwd'.
#' @examples
#' \dontrun{
#' save_OHLC("SPY")
#' }
...
# load data into list
  data <- sapply(tail(file_names), function(file_name) {
